...
BOOL in_lock = false;
ADDRINT lock_arg = -1;
long lock_enter_packets = 0;
long lock_exit_packets = 0;
...
#define fullcmp(a) (rtn_name->compare(a) == 0)
...
VOID FunEntry(ADDRINT first_arg, UINT32 encode, THREADID tid) {
	...
	//printf("[TOOL] Funentry with encode %d\n",encode  );
	//fflush(stdout);
...
void my_pthread_mutex_unlock(ADDRINT lock_address, AFUNPTR lock_function, const CONTEXT* context, THREADID ptid){

	uint64_t uarg = MASK & (uint64_t) lock_address;
	
	//printf("UNLOCK at %ld\n",uarg);
	//fflush(stdout);
	
	uint64_t time = ClockGetTime();
	THREADID tid= findThreadMapping(ptid);
	sendTimerPacket(tid,true);
	INT32 encode = UNLOCK;
	PIN_MutexLock(&mainLockForPintool);
	checkSum +=encode;
	...
	//@NOTE from akshin
	//not all programs are well formed
	//unlocking an unlocked lock is probably fine
	//if(num_unlocks > num_locks ) {

		//there is a problem ....
		//printf("PIN error with num unlocks");
		//fflush(stdout);
		//exit(1);
	//}
	...
	int ret;
	PIN_CallApplicationFunction(context,ptid,CALLINGSTD_DEFAULT,lock_function,NULL,
								PIN_PARG(int),&ret,//return type
								PIN_PARG(ADDRINT),lock_address,//argument1
								PIN_PARG_END());	
...
void my_pthread_mutex_lock(ADDRINT lock_address, AFUNPTR lock_function, const CONTEXT* context, THREADID ptid){
...
void Image(IMG img,VOID *v) {
	...
	funcRtn = RTN_FindByName(img,"pthread_mutex_lock");
	if(RTN_Valid(funcRtn)) {
		//cout<<"Lock found from image";
		RTN_Open(funcRtn);
		const string* rtn_name = new string(RTN_Name(funcRtn));
		cout<<"LOCK instrumentation for "<<  *(rtn_name) << std::endl;
		delete rtn_name;
		RTN_Close(funcRtn);

		RTN_ReplaceSignature(funcRtn,(AFUNPTR)my_pthread_mutex_lock,
							 IARG_FUNCARG_ENTRYPOINT_VALUE,0, //mutex variable
							 IARG_ORIG_FUNCPTR, // to be used with PIN call app func
							 IARG_CONST_CONTEXT, //context for registers
							 IARG_THREAD_ID,
							 IARG_END);
	...
	funcRtn = RTN_FindByName(img,"pthread_mutex_unlock");
	if(RTN_Valid(funcRtn)) {
		RTN_Open(funcRtn);
		const string* rtn_name = new string(RTN_Name(funcRtn));
		cout<<"UNLOCK instrumentation for "<<  *(rtn_name) << std::endl;
		delete rtn_name;
		RTN_Close(funcRtn);

		RTN_ReplaceSignature(funcRtn,(AFUNPTR)my_pthread_mutex_unlock,
							 IARG_FUNCARG_ENTRYPOINT_VALUE,0, //mutex variable
							 IARG_ORIG_FUNCPTR, // to be used with PIN call app func
							 IARG_CONST_CONTEXT, //context for registers
							 IARG_THREAD_ID,
...
//VOID updated_routine_instrumentation(RTN rtn, VOID* v) {
//	RTN_Open(rtn);
//	const string* rtn_name = new string(RTN_Name(rtn));

//if(cmp("pthread_mutex_lock")) {
		//need to do RTN_ReplaceSignature

		//RTN_ReplaceSignature(rtn,(AFUNPTR)my_pthread_mutex_lock,
//							 IARG_FUNCARG_ENTRYPOINT_VALUE,0, //mutex variable
//							 IARG_ORIG_FUNCPTR, // to be used with PIN call app func
//							 IARG_CONST_CONTEXT, //context for registers
//							 IARG_THREAD_ID,
...
VOID FlagRtn(RTN rtn, VOID* v) {
	RTN_Open(rtn);
	const string* rtn_name = new string(RTN_Name(rtn));
	const string* todel = rtn_name;

	//if(rtn_name->rfind("__",0) == 0) {
	//	cout<<"Starts wtih __"<<endl;
		//rtn_name->erase(0,2);
		//}

	//cout<<"Function seen =" <<*rtn_name<<endl;
	...
	if (cmp("pthread_cond_broadcast"))
		...
		encode = SIGNAL;
	//else if (cmp("pthread_mutex_lock_full")){
	/*else if (fullcmp("__pthread_mutex_lock")){

		//@TODO
		//Do a short compare and check it is not full or usercnt
		//For unlock we can also check if they only take one variable

		RTN_Close(rtn);
		//if(cmp("user") || cmp("full"))
		//   return;
	...
	//else if (cmp("pthread_mutex_unlock_full")){
	//else if (fullcmp("pthread_mutex_unlock")){
	else if (fullcmp("__pthread_mutex_unlock")){
		...
		RTN_ReplaceSignature(rtn,(AFUNPTR)my_pthread_mutex_unlock,
							 IARG_FUNCARG_ENTRYPOINT_VALUE,0, //mutex variable
							 IARG_ORIG_FUNCPTR, // to be used with PIN call app func
							 IARG_CONST_CONTEXT, //context for registers
							 IARG_THREAD_ID,
	...
	if (encode != -1 && RTN_Valid(rtn) && encode != BARRIERINIT) {
		...
		//@TODO(Akshin) @Error
		//THis is a problem
		//IARG_FUNCARG_ENTRYPOINT_VALUE is not valid at funcexit
		//Also IPOINT_AFTER is not gauranteed success
