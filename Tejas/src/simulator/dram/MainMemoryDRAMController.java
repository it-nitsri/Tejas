...
public class MainMemoryDRAMController extends MainMemoryController{
	...
	//dirty workaround for now
	 ...
	 static long totalReadAndWrite= 0L;
	 static long totalReadRank= 0L;
	 static long totalWriteRank= 0L;
	...
	//for statistics
	long totalTime = 0;
	static long totalTransactions = 0;
	 static long totalReadTransactions[][];
	 static long totalWriteTransactions[][];
	...
	public void handleEvent(EventQueue eventQ, Event e)
	{
		...
		if(e.getRequestType() == RequestType.Mem_Cntrlr_State_Update) {
			...
			//Parent cache getting set here
			b.setSimulationElement(e.getRequestingElement());
		...
		else if (e.getRequestType() == RequestType.Rank_Response)
		{
			...
			AddressCarryingEvent event = new AddressCarryingEvent(eventQ, 0,
					this, b.getSimulationElement(),	RequestType.Mem_Response,
	...
	public static void printMemoryStats(FileWriter outputFileWriter,long maxCoreCycles,long frequency) throws IOException {
		// TODO Auto-generated method stub
		for(int k=0; k < SystemConfig.mainMemoryConfig.numChans; k++)
		{
			outputFileWriter.write("For channel " + k + ":\n");
			
			//ArchitecturalComponent.getMainMemoryDRAMController(null, k).printMemoryStats(outputFileWriter,k);
			avgLatency = ArchitecturalComponent.getMainMemoryDRAMController(null,k).getAverageLatency();
			outputFileWriter.write("Average Read Latency: " + avgLatency + " cycles = " + (avgLatency/frequency * 1000) + " ns\n");
	...
	public static void resetMemoryStats(){
		...
		for(int i=0;i<SystemConfig.mainMemoryConfig.numRanks;i++){
			...
			for(int j=0;j<SystemConfig.mainMemoryConfig.numBanks;j++){
				...
				totalReadTransactions[i][j]=0L;
				totalWriteTransactions[i][j]=0L;
			    totalReadAndWrite =0L;
				totalReadRank=0L;
				totalWriteRank =0L;
				}
