...
public class EnergyConfig {
	...
	public EnergyConfig(EnergyConfig power, long numAccesses) {
		this.leakageEnergy = power.leakageEnergy * (GlobalClock.getCurrentTime()-GlobalClock.getResetTime());
	...
	public void printEnergyStats(FileWriter outputFileWriter, String componentName) throws IOException {
		outputFileWriter.write(Statistics.formatDouble(leakageEnergy) + "\t" + Statistics.formatDouble(dynamicEnergy) 
				+ "\t" + Statistics.formatDouble((leakageEnergy + dynamicEnergy)) + "\t" + Statistics.formatInt(numAccesses) + "\t" + componentName + "\n");
	}
}
