...
class ResumeSleep {
	...
	//Default initialize all resume sleep addresses to 0
	//check which one is non-zero to understand what type
	//of primitive is causing a resume and/or sleep
	
	public long barrierAddress = 0;
	public long lock_address =0; 

	//debug
	public int came_from = 0;
...
//		this.coreBcastBus = coreBcastBus;
	...
	public ResumeSleep update(long addressSynchItem, int thread, long time,
		...
		if (synchTable.containsKey(addressSynchItem)){
			...
			//if(debug) System.out.println("[UPDATE] Sync primitive found");
		}
		else {
...
//			ret = s.sigEnter(thread, time, value);
		...
		case (LOCK):
			...
			ret = s.lockEnter2(thread, time);
			ret.lock_address = addressSynchItem;
			break;
		case (UNLOCK):
			//ret = s.unlockEnter(thread, time, value);
			ret = s.unlockEnter2(thread, time, addressSynchItem);
			ret.lock_address = addressSynchItem;
			break;
		case (JOIN):
			//TODO
...
//			ret = s.waitEnter(thread, time, value);
		...
		case (LOCK + 1):
			...
			ret = s.lockExit2(thread, time, addressSynchItem);
			ret.lock_address = addressSynchItem;
			break;
		case (UNLOCK + 1):
			ret.lock_address = addressSynchItem;
			// ignore since unlock enter will block
			break;
		case (JOIN + 1):
			break;
		case (CONDWAIT + 1):
//			ret = s.waitExit(thread, time, value);
		...
		//if(ret.getNumResumers() != 0) System.out.println("Resuming something from value " + value);

		//debug causality


		
		ret.came_from = (int)value;
		return ret;
